...
public class ComputerBooksPage {
	...
	public ComputerBooksPage(SeleniumCommon selenium) throws InterruptedException {
	...
	public boolean verifySortedResults(String sFilterType,Double dblMinPrice,Double dblMaxPrice) {
		...
		case "BY_NAME":
			...
			bSorted=verifySorting(selenium.getChildElements(productContainer,compBooksLocators.
					get("cpBooksPage.ItemNames")),sFilterType,null,null);
			
			break;
		case "BY_PRICE":
			bSorted=verifySorting(selenium.getChildElements(productContainer,compBooksLocators.
					get("cpBooksPage.ItemPrices")),sFilterType,null,null);
		...
		case "BY_MIN_MAX_PRICE":
			...
			bSorted=verifySorting(selenium.getChildElements(productContainer,compBooksLocators.
					get("cpBooksPage.ItemPrices")),sFilterType,dblMinPrice,dblMaxPrice);
	...
	public boolean	 verifySorting(List<WebElement>itemList ,String sSortingBy,Double dblMinValue,Double dblMaxValue)
	{
		...
		for (int i = 1; i < itemNames.size(); i++) 
		{
			...
			{
				// if the first item has high order then second one then sorting is failed
			...
			{
				//Example :CAD18.99
				double dblPrice1=Double.parseDouble(itemNames.get(i-1).replaceAll("CAD", ""));// first item price
				double dblPrice2=Double.parseDouble(itemNames.get(i).replaceAll("CAD", ""));// 2nd item price
